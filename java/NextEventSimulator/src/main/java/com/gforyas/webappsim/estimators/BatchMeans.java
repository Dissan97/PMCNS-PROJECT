...
/**
 * Implementazione del metodo delle "Batch Means" basato sul conteggio di eventi.
 *
 * <p>
 * L'idea è dividere l'orizzonte di osservazione in N batch consecutivi, ciascuno
 * contenente un numero fissato di <b>completamenti di sistema</b> (DEPARTURE-&gt;EXIT).
 ...
 * </p>
 *
 * <p><b>Batch boundary.</b> Il confine del batch è determinato dal
 * contatore {@link CompletionsEstimator} (globale) che raggiunge {@code batchSize}.
 ...
 * esposto {@link #forwardExitToEstimators(int, double)}, con la stessa firma usata
 * in {@link StatsCollector}. Se la Simulation chiama già quel metodo su StatsCollector,
...
public class BatchMeans extends StatsCollector {
    ...
    public void calculateStats(NextEventScheduler scheduler, Network network) {
        // Non aggiunge batch parziali: se l'ultimo non ha raggiunto batchSize, lo scarta.
    ...
    /** Comodo riassunto su media e deviazione standard tra i batch delle principali metriche. */
    ...
    /** Chiamato su OGNI evento (ARRIVAL/DEPARTURE). Chiude/riapre i batch quando serve. */
    private void onAnyEvent(Event e, NextEventScheduler s) {
        if (!active || finishedAll) return;

        // Gate: batch chiuso quando i completamenti del batch corrente raggiungono batchSize
    ...
    /** Chiude il batch corrente, memorizza le statistiche, poi apre il successivo (se serve). */
    private void closeCurrentBatchAndMaybeStartNext(NextEventScheduler scheduler) {
        // 1) Finalizza al timestamp corrente per coerenza con gli stimatori time-weighted
        double now = scheduler.getCurrentTime();
        busy.finalizeBusy(now);
        pop.finalizeAt(now);
        ...
        batchPerNode.forEach((node, batch) -> {
            ...
            batch.add(new BatchStats(batchSize + 1, elapsed, compNode.get(node).getCountSinceStart(), result.sampleMean(),
                    result.sampleWait(), result.stdWn(),result.samplePopulation(), result.stdNn(), result.util()));
            busyNode.get(node).startCollecting(now);
            popNode.get(node).startCollecting(now);
            compNode.get(node).startCollecting();
            rtNode.get(node).startCollecting();
        });
        // 3) Se abbiamo raggiunto il numero desiderato di batch, chiudiamo definitivamente
