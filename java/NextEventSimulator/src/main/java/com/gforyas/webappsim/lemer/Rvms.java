package com.gforyas.webappsim.lemer;
/* -------------------------------------------------------------------------
...
public class Rvms {
    ...
    private static Rvms instance = null;

    public static Rvms getInstance() {
        if (instance == null) {
            instance = new Rvms();
        }
        return instance;
    ...
    public double cdfBinomial(long n, double p, long x)
         ...
         * ============================================
         */ {
        if (x < n) return (1.0 - inBeta(x + 1, n - x, p));
        else return (1.0);
    }
    ...
    public long idfBinomial(long n, double p, double u)
        ...
        if (cdfBinomial(n, p, x) <= u) while (cdfBinomial(n, p, x) <= u) x++;
        else if (cdfBinomial(n, p, 0) <= u) while (cdfBinomial(n, p, x - 1) > u) x--;
        else x = 0;
        return (x);
    }
    ...
    public long idfPascal(long n, double p, double u)
        ...
        if (cdfPascal(n, p, x) <= u) while (cdfPascal(n, p, x) <= u) x++;
        else if (cdfPascal(n, p, 0) <= u) while (cdfPascal(n, p, x - 1) > u) x--;
        else x = 0;
        return (x);
    }
    ...
    public long idfPoisson(double m, double u)
        ...
        if (cdfPoisson(m, x) <= u) while (cdfPoisson(m, x) <= u) x++;
        else if (cdfPoisson(m, 0) <= u) while (cdfPoisson(m, x - 1) > u) x--;
        else x = 0;
        return (x);
    }
    ...
    public double cdfStandard(double x)
        ...
        if (x < 0.0) return (0.5 * (1.0 - t));
        else return (0.5 * (1.0 + t));
    }
    ...
    public double cdfStudent(long n, double x)
        ...
        if (x >= 0.0) return (0.5 * (1.0 + s));
        else return (0.5 * (1.0 - s));
    }
    ...
    public double logChoose(long n, long m)
         ...
         * ========================================================================
         */ {
        if (m > 0) return (-logBeta(m, n - m + 1) - Math.log(m));
        else return (0.0);
    }
    ...
    public double inGamma(double a, double x)
        ...
        if (x > 0.0) factor = Math.exp(-x + a * Math.log(x) - logGamma(a));
        else factor = 0.0;
    ...
    private double rescale(double f, double[] p, double[] q) {
    ...
    public double inBeta(double a, double b, double x)
        ...
        if (x > (a + 1.0) / (a + b + 1.0)) { /* to accelerate convergence   */
            ...
            swap = false;
        if (x > 0) factor = Math.exp(a * Math.log(x) + b * Math.log(1.0 - x) - logBeta(a, b)) / a;
        else factor = 0.0;
        p[0] = 0.0;
        q[0] = 1.0;
        p[1] = 1.0;
        ...
        if (swap) return (1.0 - factor * f);
        else return (factor * f);
    }

    /**
     * Samples a Coxian(k) random variable as sum of exponentials with phase-advance Bernoulli at each phase.
     * Last phase is terminal (implicit p_k = 0).
     *
     * @param mu            phase rates [μ1..μk], μi > 0
     * @param p             continuation probs [p1..p_{k-1}], 0 <= pi <= 1
     * @param rng           RNGs manager
     * @param phaseStreamId RNG stream to draw Bernoulli phase-advance decisions
     * @param expStreamId   RNG stream to draw uniforms for exponential sampling
     * @return a Coxian-distributed sample
     */
    public double idfCoxian(double[] mu, double[] p, Rngs rng, int phaseStreamId, int expStreamId) {
        double t = 0.0;
        final int k = mu.length;
        for (int i = 0; i < k; i++) {
            // Exponential(μ_i) via inverse transform: -ln(1-U)/μ
            double u = rng.random(expStreamId);
            // Guard against log(0) if RNG returns 1.0 (shouldn't happen but be safe)
            if (u == 1.0) u = Math.nextDown(1.0);
            t += -Math.log(1.0 - u) / mu[i];

            // Last phase is terminal: break
            if (i == k - 1) break;

            // Bernoulli to decide whether to advance to next phase
            double v = rng.random(phaseStreamId);
            if (v > p[i]) {
                break; // exit after phase i
            }
    ...
    private int lastPhase = -1; // to retrieve updated phase if needed


    /**
     * Generate a single MAP inter-arrival time by simulating the CTMC until an "arrival" jump occurs.
     * Q = D0 + D1 is the generator. From state i:
     * - sojourn time ~ Exp(λ_i) with λ_i = -Q_ii
     * - jump to j with probs Q_ij / λ_i
     * - if the chosen transition comes from D1, that's an arrival; otherwise (D0) continue accumulating time.
     *
     * @param D0             silent transitions matrix
     * @param D1             arrival transitions matrix
     * @param rng            RNG manager
     * @param branchStreamId stream to choose the next branch/state
     * @param expStreamId    stream to sample exponential sojourns
     * @param phaseInOut     1-length array: phaseInOut[0] is current phase on input; updated to new phase on output
     * @return inter-arrival time > 0
     */
    public double idfMAP(double[][] D0, double[][] D1, Rngs rng, int branchStreamId, int expStreamId, int[] phaseInOut) {
        final int m = D0.length;
        int i = phaseInOut[0];
        double t = 0.0;

        while (true) {
            // 1) Sojourn rate λ_i = -Q_ii
            double qii = D0[i][i] + D1[i][i];
            double lambda = -qii; // must be > 0
            if (!(lambda > 0.0)) throw new IllegalStateException("Non-positive sojourn rate at phase " + i);

            // Exp(lambda)
            double u = rng.random(expStreamId);
            if (u == 1.0) u = Math.nextDown(1.0);
            t += -Math.log(1.0 - u) / lambda;

            // 2) Choose next state and whether transition is in D0 or D1
            // Build cumulative probabilities over all j for Q_ij / lambda
            double[] probs = new double[m];
            double cum = 0.0;
            for (int j = 0; j < m; j++) {
                if (j == i) continue; // usually Q_ii accounts only for exit rate; staying is via off-diagonals
                double qij = D0[i][j] + D1[i][j];
                if (qij < 0.0) qij = 0.0; // numeric guard
                cum += qij / lambda;
                probs[j] = cum;
            }
            // Due to numeric rounding, cum may be slightly < 1.0; cap
            for (int j = 0; j < m; j++) {
                if (probs[j] > 1.0) probs[j] = 1.0;
            }

            double v = rng.random(branchStreamId);
            int next = i;
            for (int j = 0; j < m; j++) {
                if (v <= probs[j]) {
                    next = j;
            ...
            if (next == i) {
                // If no off-diagonal chosen (degenerate numeric case), restart loop
                continue;
            }

            // 3) Decide if the chosen (i -> next) is an "arrival" transition (in D1) or silent (in D0)
            double qD0 = Math.max(0.0, D0[i][next]);
            double qD1 = Math.max(0.0, D1[i][next]);
            double qTot = qD0 + qD1;

            boolean arrival;
            if (qTot == 0.0) {
                // Should not happen if probs picked this transition, but guard
                arrival = false;
            } else {
                double w = rng.random(branchStreamId);
                arrival = (w <= qD1 / qTot);
            }

            i = next; // move to next phase

            if (arrival) {
                // Update caller's phase to the new one after the arrival transition
                phaseInOut[0] = i;
                lastPhase = i;
                return t;
            }
            // else: silent transition, continue accumulating time
        }
    }

    /**
     * Optional getter to retrieve last phase without out-param array.
     */
