...
/**
 * Stima tempo-pesata della popolazione TOTALE di sistema N(t), robusta al routing interno.
 *
 * Regole:
 *  - ARRIVAL esterno (id<0): entra SUBITO; incrementa pop e incrementa "pendingAnon".
 *  - Prima ARRIVAL con id>=0:
 *      * se "pendingAnon" > 0 → consuma uno (matching dell'anonimo) e aggiungi l'id al set, MA non incrementare pop;
 *      * altrimenti (nessun anonimo pendente) → primo ingresso con id noto → incrementa pop e registra l'id.
 *  - DEPARTURE: scala SOLO se (server, jobClass) porta a EXIT; se id>=0 rimuovi dal set.
 *
...
public class PopulationEstimator {
    ...
    // numero di job entrati come anonimi (id<0) che non hanno ancora avuto il primo id>=0
    ...
    private void tickThenMaybeEnter(Event e, NextEventScheduler s) {
        ...
        if (pendingAnon > 0) {
            // questo id chiude un "anonimo" entrato prima → non incrementare pop, ma marca l'id come presente
            pendingAnon -= 1;
            inSystem.add(id);
        } else {
